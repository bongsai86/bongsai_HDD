...
static inline uint8_t read_bu_estop(void){
  /* Pull-up 입력. 스위치 Close면 GND에 쇼트 → LOW=On */
  return (HAL_GPIO_ReadPin(ESTOP_BTN_PORT, ESTOP_BTN_PIN) == GPIO_PIN_RESET) ? 1u : 0u;
}
...
void CTRL_Tick50ms(void)
{
  ...
  if (st == ST_ESTOP){
    outputs_safe();
    rpmFR_cur=rpmFL_cur=rpmRR_cur=rpmRL_cur=0;
    MD400_SetSpeedAB_RS485(1,0,0);
    MD400_SetSpeedAB_RS485(2,0,0);
  ...
  if (st != ST_ESTOP) {
    ...
    MD400_SetSpeedAB_RS485(1, rpmRL_cur, rpmFL_cur); // ID1 = RL(M1), FL(M2)
    MD400_SetSpeedAB_RS485(2, rpmFR_cur, rpmRR_cur); // ID2 = FR(M1), RR(M2)
  }
